...
void start_UART4_DMA_RX(uint16_t length) {
    dynamixel_Ready = 0;

    LL_DMA_DisableStream(DMA1, LL_DMA_STREAM_2);
    LL_DMA_SetMemoryAddress(DMA1, LL_DMA_STREAM_2, (uint32_t)Status_Packet_Array);
    LL_DMA_SetDataLength(DMA1, LL_DMA_STREAM_2, length);
    LL_DMA_EnableStream(DMA1, LL_DMA_STREAM_2);

    LL_USART_EnableDMAReq_RX(UART4);  // 啟動 UART RX DMA
}
...
int32_t ReadPosition(uint8_t ID) {
	...
	Packet_Return = 1;
	transmitInstructionPacket();
	start_UART4_DMA_RX(Status_packet_length + 4);
	readStatusPacket();
